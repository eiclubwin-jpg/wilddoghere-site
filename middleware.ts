...
const CONTROL_STATUS_URL = "https://calmagic-is-fat.lakontratw.workers.dev/api/control/wilddoghere?v=2";
...
export async function middleware(_request: NextRequest) {
  try {
    ...
    const status = await response.json() as { active?: boolean; approved?: boolean };
    if (response.ok && status.active === true && status.approved === true) return NextResponse.next();
  } catch {
