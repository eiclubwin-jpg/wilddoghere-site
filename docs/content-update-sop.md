...
2. 每張卡片需有 `id`、`title`、`category`、`excerpt`、`platform`、`narrator`、`link`、`image`、`imageAlt`、`tags`、`status`。
3. 若正式連結尚未確定，`link` 先填 `#`，`status` 使用 `draft` 或 `coming-soon`。
4. 若作品已上線，可將 `link` 改成正式網址，並把 `status` 改成 `published`。
5. 若已有封面圖，放在 `public/images/contents/`，再更新 `image` 路徑。圖片尚未存在時，前端會自動顯示 placeholder。
